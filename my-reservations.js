...
async function loadAndRenderReservations() {
    ...
    // Show loading state
    tbody.innerHTML = '<tr><td colspan="10" style="text-align:center;padding:2rem;">Loading your reservations...</td></tr>';
    
    try {
        // Get logged-in user from session storage
        const loggedInUserStr = sessionStorage.getItem('loggedInUser');
        if (!loggedInUserStr) {
            tbody.innerHTML = '<tr><td colspan="10" style="text-align:center;padding:2rem;color:#d97757;">Please log in to view your reservations.</td></tr>';
            return;
        }
        ...
        if (!userEmail) {
            tbody.innerHTML = '<tr><td colspan="10" style="text-align:center;padding:2rem;color:#d97757;">Error: User email not found.</td></tr>';
            return;
        }
        ...
        if (activeReservations.length === 0) {
            tbody.innerHTML = '<tr><td colspan="10" style="text-align:center;padding:2rem;">No reservations found. <a href="services-list.html" style="color:var(--primary-color);text-decoration:underline;">Book your first stay!</a></td></tr>';
            return;
        }
        ...
        activeReservations.forEach(res => {
            ...
            if (res.paymentType === 'downpayment' || res.paymentType === 'down-payment') {
                ...
                if (res.paymentStatus === 'partial-payment') {
                    ...
                    paymentStatusBadgeClass = 'badge-info';
                    totalPaidAmount = downPaymentAmount;
                } else if (res.paymentStatus === 'fully-paid' || ['PAID', 'CONFIRMED', 'CHECKED_IN', 'COMPLETED'].includes(res.status)) {
            ...
            const paymentStatusBadge = '<span style="display: inline-block; padding: 4px 10px; border-radius: 4px; font-size: 0.85rem; font-weight: 600; ' +
                (paymentStatusBadgeClass === 'badge-success' ? 'background:#d4edda;color:#155724;' : paymentStatusBadgeClass === 'badge-primary' ? 'background:#dbeafe;color:#1d4ed8;' : paymentStatusBadgeClass === 'badge-info' ? 'background:#d4e5f7;color:#0066cc;' : 'background:#fff3cd;color:#856404;') +
            ...
            const hasSubmittedPayment = Boolean(res.receiptFileName) ||
                ['partial-payment', 'partially-paid', 'full-payment', 'fully-paid', 'PAID'].includes(res.paymentStatus);
            ...
            tr.innerHTML = '<td><strong>' + reservationId + '</strong></td><td>' + (res.serviceName || res.serviceType || 'N/A') + '</td><td>' + checkinDate + '</td><td>' + checkoutDate + '</td><td><span class="' + statusClass + '">' + res.status + '</span></td><td>' + paymentTypeBadge + '</td><td>' + paymentStatusBadge + '</td><td>' + totalPaid + '</td><td>' + receiptImg + '</td><td>' + actionButton + '</td>';
            tbody.appendChild(tr);
        });
        
    } catch (error) {
        console.error('Error fetching reservations:', error);
        tbody.innerHTML = '<tr><td colspan="10" style="text-align:center;padding:2rem;color:#d97757;">Failed to load reservations. ' + error.message + '<br><button onclick="location.reload()" style="margin-top:1rem;padding:0.5rem 1rem;background:var(--primary-color);color:white;border:none;border-radius:4px;cursor:pointer;">Retry</button></td></tr>';
    }
}
...
function customerCancelReservation(reservationId, serviceName) {
    ...
    const reasonServiceNameEl = document.getElementById('cancelReasonServiceName');
    ...
    submitReasonBtn.onclick = async function() {
        ...
        const composedReason = [selectedReason, additionalReason].filter(Boolean).join(' - ');
        ...
        try {
            ...
            if (loggedInUserStr) {
                try {
                    const loggedInUser = JSON.parse(loggedInUserStr);
                    const fullName = [loggedInUser.first_name, loggedInUser.middle_name, loggedInUser.last_name]
                        .filter(Boolean)
                        .join(' ')
                        .trim();
            ...
            const response = await fetch('http://localhost:3000/api/reservations/request-cancel/' + reservationId, {
                method: 'PUT',
                headers: {
