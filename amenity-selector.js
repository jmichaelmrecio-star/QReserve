...
let multipleAmenities =
  JSON.parse(sessionStorage.getItem("multipleAmenities")) || [];
...
function addAmenity(amenity) {
  ...
  const isDuplicate = multipleAmenities.some(
    (item) =>
  ...
  if (isDuplicate) {
    console.warn(
      `Amenity ${amenity.serviceName} (#${amenity.assetNumber}) is already selected`,
    );
    return false;
  }
  ...
  sessionStorage.setItem(
    "multipleAmenities",
    JSON.stringify(multipleAmenities),
  );
...
function removeAmenity(index) {
  ...
  const appliedPromo = JSON.parse(
    sessionStorage.getItem("appliedPromo") || "null",
  );
  if (appliedPromo && multipleAmenities.length > 0) {
    ...
    if (appliedPromo.discountType === "percentage") {
      appliedPromo.discountAmount =
        (newTotal * appliedPromo.discountValue) / 100;
...
function calculateTotalPrice() {
  return multipleAmenities.reduce(
...
function isAmenitySelected(serviceId, assetNumber) {
  return multipleAmenities.some(
    (item) => item.serviceId === serviceId && item.assetNumber === assetNumber,
  );
}
...
function renderSelectedItems(containerId = "selected-items-list") {
  ...
  multipleAmenities.forEach((amenity, index) => {
    ...
    const durationLabel =
      amenity.durationLabel || amenity.selectedDuration || "N/A";
    ...
    if (amenity.checkIn) {
      ...
      if (amenity.checkOut) {
        ...
        if (checkOutDate > checkInDate) {
          // Different days
          checkoutInfo = `Check-in Date: ${checkInFormatted} <br> Check-out Date: ${checkOutFormatted} <br>`;
        } else {
          // Same day
          checkoutInfo = `Check-in/out Date: ${checkInFormatted}<br>`;
        }
      ...
      if (amenity.checkOutTime && amenity.checkOutTime !== "--") {
        timeDisplay = `Check-in Time: ${timeStr} <br> Checkout Time: ${amenity.checkOutTime}`;
      } else if (timeStr) {
        timeDisplay = `Check-in/out Time: ${timeStr}`;
      }
    }
    ...
    html += `
            ...
            ">
                <div style="flex: 1;">
                    <p style="margin: 0 0 4px 0; font-weight: 600; color: var(--primary-color);">${escapeHtml(itemName)}</p>
                    <p style="margin: 0 0 4px 0; font-size: 0.85rem; color: #0066cc;">Duration: ${escapeHtml(durationLabel)}</p>
                    ${amenity.guests ? `<p style="margin: 0 0 4px 0; font-size: 0.85rem; color: #0066cc;">No. of Guests: ${amenity.guests} guest${amenity.guests > 1 ? "s" : ""}</p>` : ""}
                    ${datesDisplay ? `<p style="margin: 0 0 4px 0; font-size: 0.85rem; color: #0066cc;">${datesDisplay}${timeDisplay}</p>` : ""}
...
function updateSidebarTotal() {
  ...
  if (appliedPromo && appliedPromo.discountAmount > 0) {
    ...
    const discountedTotal = Math.max(
      0,
      originalTotal - appliedPromo.discountAmount,
    );
    ...
    if (originalPriceDisplay && originalPriceElement) {
      originalPriceElement.textContent =
        "₱" +
        originalTotal.toLocaleString("en-PH", {
          minimumFractionDigits: 2,
          maximumFractionDigits: 2,
        });
    ...
    if (discountDisplay && discountAmountElement) {
      discountAmountElement.textContent =
        "₱" +
        appliedPromo.discountAmount.toLocaleString("en-PH", {
          minimumFractionDigits: 2,
          maximumFractionDigits: 2,
        });
...
async function applyPromoCode() {
  ...
  if (!promoCode) {
    promoMessage.innerHTML =
      '<span style="color: #dc3545;">Please enter a promo code</span>';
    return;
  }

  if (multipleAmenities.length === 0) {
    promoMessage.innerHTML =
      '<span style="color: #dc3545;">Add items before applying promo code</span>';
    return;
  }
  ...
  try {
    ...
    promoMessage.innerHTML =
      '<span style="color: #dc3545;">Error validating promo code</span>';
...
function proceedToReservation() {
  if (multipleAmenities.length === 0) {
    showToast(
      "Please select at least one amenity before proceeding",
      "warning",
    );
    return;
  }
  ...
  console.log(
    "🔍 DEBUG proceedToReservation(): multipleAmenities.length =",
    multipleAmenities.length,
  );
  if (multipleAmenities.length === 1) {
    ...
    sessionStorage.setItem(
      "selectedDuration",
      singleItem.selectedDuration || singleItem.durationLabel || "",
    );
    sessionStorage.setItem(
      "selectedDurationLabel",
      singleItem.durationLabel || singleItem.selectedDuration || "",
    );
    sessionStorage.setItem("selectedAssetNumber", singleItem.assetNumber || "");

    // Store dates and times if provided
    if (singleItem.checkIn)
      sessionStorage.setItem("selectedCheckIn", singleItem.checkIn);
    if (singleItem.checkOut)
      sessionStorage.setItem("selectedCheckOut", singleItem.checkOut);
    if (singleItem.checkInTime)
      sessionStorage.setItem(
        "selectedCheckInTime",
        String(singleItem.checkInTime),
      );
    if (singleItem.checkOutTime)
      sessionStorage.setItem("selectedCheckOutTime", singleItem.checkOutTime);
    ...
    console.log(
      "✅ Redirecting to single-amenity mode:",
  ...
  const reservationData = {
    ...
    finalTotal: appliedPromo
      ? Math.max(0, calculateTotalPrice() - appliedPromo.discountAmount)
      : calculateTotalPrice(),
  ...
  console.log(
    "Proceeding to reservation with",
    multipleAmenities.length,
    "amenities",
  );
  if (appliedPromo) {
...
function initializeAmenitySelector() {
  ...
  console.log(
    "Amenity selector initialized with",
    multipleAmenities.length,
    "items",
  );
}
