...
function updatePasswordStrength() {
  ...
  const strengthMeter =
    ...
    document.getElementById("passwordStrengthContainer"); // Simplified
  const strengthText =
  ...
  const { requirements, strength, metCount } =
  ...
  Object.keys(requirements).forEach((req) => {
    ...
    if (el) {
      el.style.color = requirements[req]
        ? "var(--success-color)"
        : "var(--danger-color)";
    }
  });
...
async function loginUser(event) {
  ...
  const emailInput =
    document.getElementById("email") || document.getElementById("loginEmail");
  const passwordInput =
    document.getElementById("password") ||
...
function checkLoginStatusForReservation() {
  ...
  const detailsSection =
    document.getElementById("customer-details-section") ||
  ...
  if (user) {
    ...
    if (detailsSection) {
      ...
      const phoneInput =
        ...
        document.getElementById("customer_contact");
      const addressInput =
        document.getElementById("address") ||
        document.getElementById("customer_address");
      const birthdayInput =
        document.getElementById("birthday") ||
        document.getElementById("customer_birthday");
...
async function renderUserReservations() {
  ...
  try {
    const response = await fetch(
      `http://localhost:3000/api/reservations/user/${user.email}`,
      {
    ...
    if (data.success || Array.isArray(data)) {
      ...
      if (!reservations || reservations.length === 0) {
        ...
        reservations.forEach((res) => {
          ...
          const canViewQR =
            status !== "PENDING" && status !== "CART" && status !== "CANCELLED";
          const qrLink = hash
            ? canViewQR
              ? `<a href="confirmation.html?hash=${hash}" style="display: inline-block; padding: 6px 12px; background-color: var(--primary-color); color: white; text-decoration: none; border-radius: 4px; font-weight: 500; font-size: 0.9rem; transition: background-color 0.3s; border: none; cursor: pointer;" onmouseover="this.style.backgroundColor=getComputedStyle(document.documentElement).getPropertyValue('--primary-hover')" onmouseout="this.style.backgroundColor=getComputedStyle(document.documentElement).getPropertyValue('--primary-color')">View QR</a>`
        ...
        document.querySelectorAll(".checkout-btn").forEach((btn) => {
          btn.addEventListener("click", async function () {
            ...
            if (!resId) return;
            if (!confirm("Are you sure you want to checkout this reservation?"))
              return;
            try {
              const response = await fetch(
                `http://localhost:3000/api/reservations/update-status/${resId}`,
                {
                  method: "PUT",
                  headers: {
    ...
    listElement.innerHTML =
      '<tr><td colspan="6" style="text-align: center; color: var(--error-color);">Network error. Please check your connection.</td></tr>';
  }
}
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  if (
    ...
    document.getElementById("user-info-display")
  ) {
    renderProfileDetails();
  }
