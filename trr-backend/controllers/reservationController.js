...
async function validatePromoCodeUsage(code, baseAmount) {
  ...
  if (promoCode.minPurchaseAmount && baseAmount < promoCode.minPurchaseAmount) {
    return {
      ok: false,
      message: `This promo code requires a minimum purchase of ₱${promoCode.minPurchaseAmount.toLocaleString()}.`,
    };
  }
...
async function applyPromoUsageForReservations(reservations) {
  ...
  const alreadyApplied = reservations.some(
    (reservation) => reservation.promoUsageApplied,
  );
  if (alreadyApplied) {
    return { ok: true };
  }
  ...
  const promoReservation =
    reservations.find((reservation) => reservation.discountCode) ||
  ...
  const incremented = await incrementPromoUsage(
    promoCode._id,
...
async function validateLeadTime(
  ...
  try {
    ...
    const isVenue =
      serviceCategory.toLowerCase().includes("event_space") ||
...
async function validateRescheduleAvailability(
  reservations,
  checkInDateTime,
  checkOutDateTime,
  userEmail,
  ...
  try {
    ...
    const daysUntilCheckIn = Math.ceil(
      (proposedCheckIn - now) / (1000 * 60 * 60 * 24),
    );

    if (daysUntilCheckIn < 14) {
      return {
        ok: false,
        message: `Rescheduling requires a minimum 14-day lead time. You have ${daysUntilCheckIn} days. Please choose a date at least 14 days from now.`,
      };
    }
    ...
    if (blockedDates) {
      return {
        ok: false,
        message: `The new dates fall on blocked/maintenance dates. Please choose dates when the resort is open.`,
      };
    }
...
async function createPrivatePoolGlobalLock(reservation) {
  try {
    ...
    console.log(
      "🔍 DEBUG - Reservation object keys:",
      Object.keys(reservation.toObject ? reservation.toObject() : reservation),
    );
    console.log(
      "🔍 DEBUG - Reservation email field:",
      reservation.email,
      "full_name:",
    ...
    console.log(
      "🔒 Creating global resort lock for Private Pool reservation:",
      {
        reservationId: reservation.reservationId,
...
async function removePrivatePoolGlobalLock(reservation) {
  try {
    ...
    console.log(
      "🔓 Removing global resort lock for cancelled Private Pool reservation:",
      {
        reservationId: reservation.reservationId,
      },
    );
...
exports.checkCartDuplicate = async (req, res) => {
  try {
    ...
    if (existing) {
      return res
        ...
        .json({
          ...
          duplicate: true,
          message:
            "You already have a reservation or cart item for this service and date range.",
        });
    }
    ...
    res
      ...
      .json({
        success: false,
        message: "Failed to check for duplicate cart item.",
      });
  }
};
// --- CART API ENDPOINTS ---
  ...
  try {
    ...
    if (!reservationId || !subject || !body) {
      return res
        ...
        .json({
          success: false,
          message: "Missing reservationId, subject, or body.",
        });
    }
    ...
    if (!reservation || !reservation.email) {
      return res
        ...
        .json({
          success: false,
          message: "Reservation or customer email not found.",
        });
    }
    ...
    try {
      ...
      console.log(
        `✅ Custom email sent to ${reservation.email} for reservation ${reservation.reservationId}`,
      );
      res
        .status(200)
      ...
      res.status(200).json({
        success: true,
        message:
          "Email service unavailable. Your message has been saved and will be sent when service is restored.",
      });
    }
  } catch (error) {
    ...
    res
      .status(500)
      .json({ success: false, message: "Failed to process email request." });
  }
};
...
exports.getAllReservations = async (req, res) => {
  try {
    ...
    res
      ...
      .json({
        success: false,
        message: "Server error fetching all reservations.",
        error: error.message,
      });
  }
};
...
async function generateFormalReservationId() {
  ...
  console.log(
    `🆔 Generated Formal Reservation ID: ${newId} (Next in sequence after ${existingSequences[0] || 0})`,
  );
  return newId;
}
...
exports.createReservation = async (req, res) => {
  try {
    console.log(
      "📍 createReservation called with body:",
    ...
    if (req.user) {
      ...
      if (!fullName || !email || !contactNumber) {
        return res
          ...
          .json({
            success: false,
            message: "Guest reservations require full contact details.",
          });
      }
    }
    ...
    if (isNaN(checkIn.getTime()) || isNaN(checkOut.getTime())) {
      return res
        ...
        .json({
          success: false,
          message: "Invalid date format received for check-in or check-out.",
        });
    }
    ...
    if (checkIn < today || checkOut < today || checkOut <= checkIn) {
      return res
        ...
        .json({
          success: false,
          message:
            "Invalid dates. Check-in and Check-out must be future dates, and check-out must be after check-in.",
        });
    }

    // --- B2. Validate Lead Time Requirements ---
    console.log(
      "⏰ Validating lead time requirements for serviceId:",
      serviceId,
    );
    const leadTimeCheck = await validateLeadTime(
      serviceId,
      checkIn,
      serviceType,
    );
    ...
    for (const block of blockedDates) {
      ...
      const serviceIds = Array.isArray(block.serviceIds)
        ? block.serviceIds.map((id) => id.toString())
        : [];
      const appliesToAll =
        typeof block.appliesToAllServices === "boolean"
      ...
      const isServiceBlocked =
        appliesToAll || serviceIds.includes(serviceId?.toString());
    ...
    if (existingReservations.length > 0) {
      ...
      const existingCheckinStr = new Date(
        conflictingReservation.check_in,
      ...
      const existingCheckoutStr = new Date(
        conflictingReservation.check_out,
      ).toLocaleString("en-US", {
        year: "numeric",
        month: "short",
        day: "numeric",
    ...
    if (numberOfGuests > service.max_guests) {
      return res
        ...
        .json({
          success: false,
          message: `The chosen service allows a maximum of ${service.max_guests} guests.`,
        });
    }
    ...
    if (!durationOrSlot) {
      return res
        ...
        .json({
          success: false,
          message: "Duration or time slot selection is required.",
        });
    }
    ...
    if (!priceData) {
      return res
        ...
        .json({
          success: false,
          message:
            "Invalid duration/time slot or guest count for the selected service.",
        });
    }

    // Server-side price validation (prevent frontend manipulation)
    const expectedBasePrice = priceData.price;
    const expectedFinalTotal = discountValue
      ? expectedBasePrice - discountValue
      : expectedBasePrice;

    const promoValidation = await validatePromoCodeUsage(
      discountCode,
      expectedBasePrice,
    );
    if (!promoValidation.ok) {
      return res
        .status(400)
        .json({ success: false, message: promoValidation.message });
    }
    const appliedPromo = promoValidation.promo;

    console.log("💵 Price validation:", {
      expectedBasePrice,
      receivedBase: basePrice,
      expectedFinalTotal,
      receivedFinal: finalTotal,
    });

    // Allow 1 peso tolerance for rounding differences
    if (
      Math.abs(parseFloat(basePrice) - expectedBasePrice) > 1 ||
      Math.abs(parseFloat(finalTotal) - expectedFinalTotal) > 1
    ) {
      console.error(
        `❌ Price mismatch: Expected base=${expectedBasePrice}, received=${basePrice}; Expected final=${expectedFinalTotal}, received=${finalTotal}`,
      );
      return res.status(400).json({
        success: false,
    ...
    console.log("✅ Generated IDs:", {
      reservationHash: reservationHash.substring(0, 8) + "...",
      formalReservationId,
    });
...
exports.createMultiAmenityReservation = async (req, res) => {
  try {
    console.log(
      "📍 createMultiAmenityReservation called with body:",
      JSON.stringify(req.body, null, 2),
    );
    ...
    if (
      ...
      return res.status(400).json({
        success: false,
        message:
          "Invalid request: customer details and amenities array are required.",
      });
    }
    ...
    for (let i = 0; i < amenities.length; i++) {
      ...
      const blockedDates = await BlockedDate.find();
      console.log(
        `🔍 Checking ${blockedDates.length} blocked dates for amenity #${i + 1} (${amenity.serviceName})`,
      );
      console.log(
        `   Requested dates: ${checkIn.toISOString()} to ${checkOut.toISOString()}`,
      );
      ...
      for (const block of blockedDates) {
        ...
        const isServiceBlocked =
          appliesToAll || serviceIds.includes(amenity.serviceId?.toString());

        console.log(
          `   Block: ${blockStart.toISOString()} to ${blockEnd.toISOString()}`,
        );
        console.log(
          `   - appliesToAll: ${appliesToAll}, serviceIds: ${serviceIds.join(",")}`,
        );
          ...
          if (isPrivatePoolBlock) {
            const isPoolBooker =
              (accountId &&
                block.reservedByAccountId &&
                accountId.toString() ===
                  block.reservedByAccountId.toString()) ||
              (email &&
                block.reservedByEmail &&
                email.toLowerCase() === block.reservedByEmail.toLowerCase());
            ...
            if (isPoolBooker) {
              console.log(
                "✅ Allowing pool booker to reserve amenities (multi):",
                { accountId, email },
              );
              continue; // Skip this block since they booked the pool
            }
      ...
      if (existingReservations.length > 0) {
        ...
        const existingCheckinStr = new Date(conflict.check_in).toLocaleString(
          "en-US",
          {
            year: "numeric",
            month: "short",
        ...
        const existingCheckoutStr = new Date(conflict.check_out).toLocaleString(
          "en-US",
          {
            year: "numeric",
            month: "short",
    ...
    for (let i = 0; i < amenities.length; i++) {
      ...
      if (!leadTimeCheck.valid) {
        console.log(
          `❌ Lead time validation failed for amenity #${i + 1}:`,
          leadTimeCheck.message,
        );
        return res.status(400).json({
          success: false,
    ...
    for (let i = 0; i < amenities.length; i++) {
      ...
      const newReservation = new Reservation({
        ...
        serviceName:
          amenity.serviceName ||
          (await getServiceName(amenity.serviceId)) ||
          "N/A", // Store service name for easier display
        full_name: name,
        check_in: checkIn,
        check_out: checkOut,
        ...
        checkInTimeSlot: amenity.checkInTime
          ? String(amenity.checkInTime)
          : null,
        checkOutTimeSlot: amenity.checkOutTime
          ? String(amenity.checkOutTime)
          : null,
      });
    ...
    res.status(201).json({
      success: true,
      message:
        "Multi-amenity reservation created successfully. Proceed to payment.",
...
exports.finalizeReservation = async (req, res) => {
  ...
  if (!reservationHash || !gcashReferenceNumber) {
    return res
      ...
      .json({
        success: false,
        message: "Missing hash or GCash reference number in the request body.",
      });
  }
  ...
  if (!receiptImage) {
    return res
      .status(400)
      .json({ success: false, message: "Payment receipt image is required." });
  }
  ...
  try {
    ...
    if (!reservation) {
      return res
        ...
        .json({
          success: false,
          message: "Reservation not found using the provided hash.",
        });
    }
    ...
    if (
      ...
      return res
        ...
        .json({
          success: false,
          message:
            "Reservation is already confirmed or payment has been processed.",
        });
    }
...
exports.getPendingReservations = async (req, res) => {
  try {
    ...
    console.error(
      "SERVER ERROR fetching pending reservations for Admin:",
      error,
    );
    res.status(500).json({
...
exports.getReservationDetails = async (req, res) => {
  try {
    ...
    if (reservation.reservationHash !== hash) {
      return res
        .status(404)
        .json({ message: "Reservation not found or invalid access." });
    }
...
exports.updateReservationStatus = async (req, res) => {
  try {
    ...
    if (
      currentReservation.isMultiAmenity &&
      currentReservation.multiAmenityGroupId
    ) {
    ...
    if (normalizedStatus === "CANCELLED") {
      ...
      ["CONFIRMED", "CHECKED_IN", "COMPLETED"].includes(normalizedStatus)
    ) {
      const promoUsageResult =
        await applyPromoUsageForReservations(reservationsToUpdate);
  ...
  try {
    ...
    if (!proposedCheckIn || !proposedCheckOut) {
      return res
        .status(400)
        .json({
          message: "Proposed check-in and check-out dates are required.",
        });
    }
    ...
    if (
      ...
      return res
        .status(403)
        .json({
          message: "You are not authorized to reschedule this reservation.",
        });
    }
    ...
    if (
      isNaN(proposedCheckInDate.getTime()) ||
      isNaN(proposedCheckOutDate.getTime())
    ) {
    ...
    if (
      earliestCheckIn &&
      earliestCheckIn.getTime() - now.getTime() < minRescheduleMs
    ) {
      return res.status(400).json({
        message:
          "Reschedules must be requested at least 14 days before the check-in date.",
      });
    }
...
exports.approveReschedule = async (req, res) => {
  try {
    ...
    if (
      reservation.rescheduleStatus !== "PENDING" ||
      !reservation.rescheduleProposedCheckIn ||
      !reservation.rescheduleProposedCheckOut
    ) {
      return res
        .status(400)
        .json({ message: "No pending reschedule request found." });
    }
    ...
    try {
      ...
      console.log(
        `✅ Approval email sent to ${customerEmail} for reservation ${reservation.reservationId}`,
      );
    } catch (emailError) {
      console.error(
        `⚠️ Failed to send approval email to ${customerEmail}:`,
        emailError.message,
      );
...
exports.rejectReschedule = async (req, res) => {
  try {
    ...
    try {
      ...
      console.log(
        `✅ Rejection email sent to ${customerEmail} for reservation ${reservation.reservationId}`,
      );
    } catch (emailError) {
      console.error(
        `⚠️ Failed to send rejection email to ${customerEmail}:`,
        emailError.message,
      );
      // Don't fail the request if email fails, just log it
    }
...
exports.blockDateForMaintenance = async (req, res) => {
  try {
    ...
    if (!date || !reason) {
      return res
        .status(400)
        .json({ message: "Missing date or reason for blocking." });
    }
    ...
    if (existingBlock) {
      return res
        .status(400)
        .json({ message: `Date ${date} is already blocked for maintenance.` });
    }
  ...
  try {
    ...
    if (!hashFromUrl) {
      ...
      return res
        ...
        .json({
          success: false,
          message: "Reservation Hash is missing from the URL.",
        });
    }

    // --- 2. Find Reservation by hash OR reservationId ---
    let reservation = await Reservation.findOne({
      reservationHash: hashFromUrl,
    });
    ...
    if (!reservation) {
      return res
        ...
        .json({
          success: false,
          message: "No matching reservation found for this hash or ID.",
        });
    }
  ...
  try {
    ...
    if (!startDate || !endDate) {
      return res
        .status(400)
        .json({
          message: "Start and End dates are required for report generation.",
        });
    }
    ...
    res.setHeader(
      "Content-Disposition",
      `attachment; filename="tito_renz_reservation_report_${startDate}_to_${endDate}.pdf"`,
    );
    ...
    doc
      .fontSize(24)
      .font("Helvetica-Bold")
      .text("TITO RENZ RESORT", { align: "center" });
    doc
      .fontSize(14)
      .font("Helvetica")
    ...
    doc.text(`Generated By: ${generatedBy || "Unknown User"}`, {
      align: "center",
    });
    if (serviceType && serviceType !== "all") {
      doc.text(`Service Type Filter: ${serviceType.toUpperCase()}`, {
        align: "center",
      });
    }
    doc.moveDown(0.5);
    ...
    doc.moveDown(0.2);
    doc.fontSize(10).font("Helvetica");
    doc.text(`Total Completed Reservations: ${totalReservations}`, {
      indent: 20,
    });
    doc.text(
      `Total Income: PHP ${totalIncome.toLocaleString("en-US", { minimumFractionDigits: 2, maximumFractionDigits: 2 })}`,
      { indent: 20 },
    );
    doc.moveDown(0.4);
    ...
    if (Object.keys(serviceBreakdown).length > 0) {
      doc
        .fontSize(12)
        .font("Helvetica-Bold")
        .text("BREAKDOWN BY SERVICE TYPE", { underline: true });
      doc.moveDown(0.2);
      ...
      for (const [serviceType, data] of Object.entries(serviceBreakdown)) {
        ...
        doc.text(
          `PHP ${data.revenue.toLocaleString("en-US", { minimumFractionDigits: 2, maximumFractionDigits: 2 })}`,
          colX[2],
        );
        doc.moveDown(0.2);
      }
    ...
    if (reservations.length > 0) {
      ...
      doc
        .fontSize(12)
        .font("Helvetica-Bold")
        .text("DETAILED RESERVATION LIST", { underline: true });
      doc.moveDown(0.3);
...
exports.getReservationByHash = async (req, res) => {
  try {
    ...
    if (!hash) {
      return res
        .status(400)
        .json({ success: false, message: "Reservation hash is required." });
    }
    ...
    if (hashes.length === 0) {
      return res
        ...
        .json({
          success: false,
          message: "No valid reservation hashes provided.",
        });
    }

    const reservation = await Reservation.findOne({
      reservationHash: { $in: hashes },
    });
    ...
    if (!reservation) {
      ...
      return res
        ...
        .json({
          success: false,
          message: "Reservation not found or invalid hash.",
        });
    }
  ...
  try {
    ...
    if (blockedDates.length > 0) {
      ...
      if (isPrivatePoolBlock) {
        ...
        if (isPoolBooker) {
          console.log("✅ Allowing pool booker to check availability:", {
            accountId,
            userEmail,
          });
...
exports.checkServiceTypesAvailability = async (req, res) => {
  try {
    ...
    if (
      ...
      return res.status(400).json({
        message:
          "Missing required fields: checkInDate, checkInTime, checkOutDate, serviceTypes",
        availableServices: [],
      });
    }
    ...
    const checkinDateTime = new Date(
      `${checkInDate}T${String(parseInt(checkInTime, 10)).padStart(2, "0")}:00:00`,
    );
    const checkoutDateTime = checkOutTime
      ? new Date(
          `${checkOutDate}T${String(parseInt(checkOutTime, 10)).padStart(2, "0")}:00:00`,
        )
      ...
      const blockedForService = blockedDates.some((bd) => {
        if (bd.appliesToAllServices) {
          ...
          if (bd.reason && bd.reason.includes("Private Pool Area Reserved")) {
            ...
            if (isPoolBooker) {
              console.log(`✅ Pool booker can access service ${serviceId}:`, {
                accountId,
                userEmail,
              });
            ...
            console.log(
              `🔒 Service ${serviceId} blocked by Private Pool reservation: ${bd.reason}`,
            );
          }
          return true;
        }
        if (!bd.serviceIds || bd.serviceIds.length === 0) return false;
        return bd.serviceIds.some(
          (bdServiceId) =>
...
exports.getPendingPaymentVerifications = async (req, res) => {
  try {
    ...
    const pendingPayments = await Reservation.find({
      paymentStatus: {
        $in: ["PENDING", "partial-payment", "partially-paid", "full-payment"],
      },
    ...
    console.error(
      "SERVER ERROR fetching pending payment verifications:",
      error,
    );
    res.status(500).json({
  ...
  try {
    ...
    if (reservation.paymentStatus !== "PENDING") {
      return res
        .status(400)
        .json({
          message: `Payment for reservation ${id} is already ${reservation.paymentStatus}.`,
        });
    }

    const resolvedPaymentStatus =
      paymentStatus ||
      (reservation.paymentType === "full" ? "PAID" : "DOWNPAYMENT_PAID");
    ...
    try {
      const hashes = reservationsToUpdate
        .map((r) => r.reservationHash)
        .filter(Boolean);
  ...
  try {
    ...
    if (!rejectableStatuses.includes(reservation.paymentStatus)) {
      return res
        .status(400)
        .json({
          message: `Cannot reject payment for reservation ${id} with status: ${reservation.paymentStatus}`,
        });
    }
    ...
    try {
      ...
      console.error(
        "Email sending failed for rejection notification:",
        emailError.message,
      );
    }
  ...
  try {
    ...
    if (!["PENDING", "partial-payment"].includes(reservation.paymentStatus)) {
      return res
        .status(400)
        .json({
          message: `Cannot approve partial payment for reservation ${id} with status: ${reservation.paymentStatus}`,
        });
    }
    ...
    try {
      ...
      console.error(
        "Email sending failed for partial approval:",
        emailError.message,
      );
    }
...
exports.approveFullPayment = async (req, res) => {
  try {
    ...
    if (
      !["PENDING", "full-payment", "partially-paid"].includes(
      ...
      return res
        .status(400)
        .json({
          message: `Cannot approve full payment for reservation ${id} with status: ${reservation.paymentStatus}`,
        });
    }
    ...
    try {
      ...
      console.error(
        "Email sending failed for full payment approval:",
        emailError.message,
      );
    }
  ...
  try {
    ...
    if (reservations.length === 0) {
      return res
        ...
        .json({
          success: false,
          message: "No reservations found for the provided hash(es).",
        });
    }
    ...
    if (multiAmenityGroupIds.length > 0) {
      ...
      console.log(
        `📦 Multi-amenity group detected. Updating ${allReservationsToUpdate.length} items in group(s).`,
      );
    }
    ...
    const updatePromises = allReservationsToUpdate.map(async (reservation) => {
      ...
      if (hashes.length === 1) {
        if (downpaymentAmount) {
          ...
          const cleanDownpayment = parseFloat(
            downpaymentAmount.toString().replace(/,/g, ""),
          );
          if (!isNaN(cleanDownpayment))
            reservation.downpaymentAmount = cleanDownpayment;
        }
        if (remainingBalance) {
          const cleanBalance = parseFloat(
            remainingBalance.toString().replace(/,/g, ""),
          );
          if (!isNaN(cleanBalance)) reservation.remainingBalance = cleanBalance;
        }
        if (totalAmount) {
          const cleanTotal = parseFloat(
            totalAmount.toString().replace(/,/g, ""),
          );
    ...
    console.log(
      `✅ Payment receipt processed for ${hashes.length} reservation(s):`,
      hashes,
    );
    ...
    res
      ...
      .json({
        success: false,
        message: "Internal server error during receipt upload.",
      });
  }
};
