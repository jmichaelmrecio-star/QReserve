...
function getAuthToken() {
    try {
        ...
        console.log('🔑 Auth Token Retrieved:', token ? '✓ Token found' : '✗ Token NOT found');
        return token || '';
    } catch (err) {
...
async function renderRescheduleRequests() {
    ...
    tbody.innerHTML = '<tr><td colspan="8" class="text-center p-4">Loading reschedule requests...</td></tr>';
    ...
    try {
        ...
        const rescheduleRequests = reservations.filter(r => 
            r.rescheduleStatus === 'PENDING' || 
            (r.rescheduleProposedCheckIn && r.rescheduleStatus !== 'REJECTED' && r.rescheduleStatus !== 'APPROVED')
        );

        if (rescheduleRequests.length === 0) {
            tbody.innerHTML = '<tr><td colspan="8" class="text-center p-4">No pending reschedule requests.</td></tr>';
            return;
        }

        tbody.innerHTML = rescheduleRequests.map(res => {
            const currentCheckIn = res.check_in ? new Date(res.check_in).toLocaleDateString() : 'N/A';
            const currentCheckOut = res.check_out ? new Date(res.check_out).toLocaleDateString() : 'N/A';
            const proposedCheckIn = res.rescheduleProposedCheckIn ? new Date(res.rescheduleProposedCheckIn).toLocaleDateString() : 'N/A';
            const proposedCheckOut = res.rescheduleProposedCheckOut ? new Date(res.rescheduleProposedCheckOut).toLocaleDateString() : 'N/A';
            const requestedAt = res.rescheduleRequestedAt ? new Date(res.rescheduleRequestedAt).toLocaleString() : 'N/A';
        ...
        tbody.innerHTML = '<tr><td colspan="8" class="text-center text-danger">Error loading reschedule requests.</td></tr>';
...
function showApproveModal(reservationId, reservationIdForDisplay) {
    ...
    const confirmApproveMessage = document.getElementById('confirmApproveMessage');
...
async function approveRescheduleRequest() {
    ...
    try {
        ...
        console.log('📤 Sending approve request with token:', token ? `✓ (${token.substring(0, 20)}...)` : '✗ EMPTY');
        ...
        const response = await fetch(
            `http://localhost:3000/api/reservations/${reservationId}/approve-reschedule`,
            {
                ...
                body: JSON.stringify({
                    approvedBy: JSON.parse(sessionStorage.getItem('loggedInUser') || '{}').email
                })
            }
        );
        ...
        if (response.ok) {
            ...
            const confirmApproveModal = bootstrap.Modal.getInstance(document.getElementById('confirmApproveModal'));
        ...
        showResultModal('Error', `Failed to approve reschedule: ${error.message}`, 'error');
    }
}
...
async function rejectRescheduleRequest() {
    ...
    if (!rejectionReason) {
        showResultModal('Input Required', 'Please provide a reason for rejecting this reschedule request.', 'warning');
        return;
    }
    ...
    try {
        ...
        const response = await fetch(
            `http://localhost:3000/api/reservations/${reservationId}/reject-reschedule`,
            {
                ...
                body: JSON.stringify({
                    rejectedBy: JSON.parse(sessionStorage.getItem('loggedInUser') || '{}').email,
        ...
        if (response.ok) {
            ...
            const rejectReasonModal = bootstrap.Modal.getInstance(document.getElementById('rejectReasonModal'));
        ...
        showResultModal('Error', `Failed to reject reschedule: ${error.message}`, 'error');
    }
}
