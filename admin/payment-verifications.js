...
function formatPaymentDetails(reservation) {
  const receiptUrl = reservation.receiptFileName
    ? `http://localhost:3000/uploads/${reservation.receiptFileName}`
    : "";
  ...
  const uploadedDate = reservation.receiptUploadedAt
    ? new Date(reservation.receiptUploadedAt).toLocaleString()
    : "N/A";
  ...
  if (reservation.paymentType === "fullpayment") {
    paymentTypeBadge =
      '<span class="badge bg-success">100% Full Payment</span>';
  }
  ...
  const submittedAmount =
    reservation.paymentType === "downpayment" ? downPaymentAmount : totalAmount;
  const paidAmount =
...
function togglePaymentDetails(btn, paymentId) {
  ...
  if (nextRow && nextRow.classList.contains("payment-details-row")) {
    ...
    const payment = pendingPaymentsData.find(
      (p) => (p._id || p.reservationId) === paymentId,
    );
    if (payment) {
...
async function renderPendingPayments() {
  ...
  tbody.innerHTML =
    '<tr><td colspan="8" class="text-center">Loading pending payments...</td></tr>';
  ...
  try {
    ...
    if (!Array.isArray(pending) || pending.length === 0) {
      tbody.innerHTML =
        '<tr><td colspan="9" class="text-center">No pending payments.</td></tr>';
      return;
    }
    ...
    if (tbody.children.length === 0) {
      tbody.innerHTML =
        '<tr><td colspan="9" class="text-center">No matching reservations found.</td></tr>';
    }
  } catch (error) {
...
function renderSingleReservations(tbody, reservations) {
    ...
    .forEach((reservation) => {
      ...
      const paymentAmount =
        reservation.paymentType === "downpayment"
          ? totalAmount * 0.5
          : totalAmount;
      const amount = paymentAmount.toLocaleString("en-PH", {
        minimumFractionDigits: 2,
      });
      const uploaded = reservation.receiptUploadedAt
        ? new Date(reservation.receiptUploadedAt).toLocaleString()
        : "N/A";
      ...
      if (reservation.paymentStatus === "partial-payment") {
        ...
        paymentStatusBadge =
          '<span class="badge bg-primary">50% Approved</span>';
      } else if (reservation.paymentStatus === "full-payment") {
        paymentStatusBadge =
          '<span class="badge bg-info">100% Submitted</span>';
      } else if (reservation.paymentStatus === "fully-paid") {
        paymentStatusBadge =
  ...
  tbody
    .querySelectorAll('button[data-action]:not([data-action*="group"])')
    .forEach((btn) => {
...
function renderMultiAmenityGroups(tbody, groups) {
  groups.forEach((group, groupIndex) => {
    ...
    const totalAmount = group.reservations.reduce(
      (sum, r) => sum + Number(r.finalTotal || 0),
      0,
    );
    // Calculate actual payment amount based on payment type
    const paymentAmount =
      firstRes.paymentType === "downpayment" ? totalAmount * 0.5 : totalAmount;
    const uploaded = firstRes.receiptUploadedAt
      ? new Date(firstRes.receiptUploadedAt).toLocaleString()
      : "N/A";

    // Create list of service names for multi-amenity
    const servicesList = group.reservations
      .map((r) => r.serviceName || r.serviceType || "Unknown")
      .join(", ");
    ...
    const approveButtonClass =
      firstRes.paymentStatus === "partially-paid" ? "btn-info" : "btn-success";
  ...
  tbody.querySelectorAll("button[data-action]").forEach((btn) => {
    if (
      ...
      btn.getAttribute("data-action").includes("group")
    ) {
      btn.addEventListener("click", async function () {
        const action = this.getAttribute("data-action");
...
function toggleMultiAmenityGroup(btn, groupIndex) {
  ...
  if (nextRow && nextRow.classList.contains("multi-amenity-details-row")) {
    ...
    if (group && group.reservations) {
      ...
      detailsHTML +=
        '<h6 class="mb-3">📋 Reservations in this Multi-Amenity Booking:</h6>';
      detailsHTML +=
        '<div style="margin-bottom: 10px; padding: 8px 10px; border-radius: 6px; background: #fff3cd; color: #856404; font-size: 0.9rem;">Approvals are bundle-wide. Use Approve All/Reject All to keep the booking consistent.</div>';
      detailsHTML += '<table class="table table-sm table-bordered">';
      detailsHTML +=
        "<thead><tr><th>Reservation ID</th><th>Service</th><th>Duration</th><th>Check-In</th><th>Amount</th></tr></thead><tbody>";
      ...
      group.reservations.forEach((res) => {
        ...
        const checkIn = res.check_in
          ...
          : "N/A";
        const amount = Number(res.finalTotal || 0).toLocaleString("en-PH", {
          minimumFractionDigits: 2,
        });
...
async function processGroupPaymentAction(
  buttonEl,
  groupIndex,
  action,
  confirmMsg,
  ...
  console.log(
    "🔐 DEBUG - handleGroupPaymentAction: Token length:",
    token.length,
    "Token exists:",
    !!token,
    "Action:",
  ...
  try {
    ...
    console.log("🚀 DEBUG - Multi-Amenity Fetch:", {
      endpoint,
      method: "PATCH",
    ...
    if (response.ok) {
      ...
      showToast(
        `All ${group.reservations.length} reservations ${actionMsg} successfully!`,
        "success",
      );
    } else {
...
async function handlePaymentAction(reservationId, action, buttonEl) {
  ...
  console.log(
    "🔐 DEBUG - handlePaymentAction: Token length:",
    token.length,
    "Token exists:",
    !!token,
    "Reservation:",
  ...
  if (action === "approve-partial") {
    ...
    payload = { status: "REJECTED", paymentStatus: "REJECTED" };
    confirmMsg =
      "Reject this payment? The reservation will be marked as rejected.";
  } else {
    // Legacy support
  ...
  // Always show confirmation for all actions
  if (typeof window.showConfirm !== "function") {
    showToast(
      "Confirmation dialog is unavailable. Please refresh and try again.",
      "warning",
  ...
  window.showConfirm(
    ...
    {
      confirmText: action === "reject" ? "Reject" : "Approve",
      cancelText: "Cancel",
...
async function processSinglePaymentAction(
  buttonEl,
  reservationId,
  action,
  endpoint,
  ...
  try {
    console.log("🚀 DEBUG - Payment Fetch:", {
      endpoint,
      method: "PATCH",
      token: `Bearer ${token.substring(0, 20)}...`,
      payload,
