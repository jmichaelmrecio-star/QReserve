  ...
  async function validateDates() {
    ...
    try {
      ...
      for (const block of blockedDates) {
        ...
        if (isServiceBlocked) {
          ...
          if (checkinDate <= blockEnd && checkoutDate >= blockStart) {
            ...
            showToast(
              `This service is unavailable from ${blockStartStr} to ${blockEndStr}. Reason: ${block.reason}. Please select different dates.`,
              "error",
            );
            checkinInput.value = "";
            checkoutInput.value = "";
      ...
      if (!availabilityResult.available) {
        showToast(
          "This service is already booked for the selected dates. Please choose different dates or another service.",
          "error",
        );
        checkinInput.value = "";
        checkoutInput.value = "";
